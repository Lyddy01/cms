  ...
  if (isset($_POST['submit'])) {
    ...
    if ($auth->checkRows($login) == 0) {
      ...
      if (isset($_GET['redirect']) || !empty($_GET['redirect'])) {
        ...
        ?>
        <script>window.location = "https://lydiacms.000webhostapp.com/post/<?php echo $posts->preetyUrl($_GET['id']) ?>/<?php echo $posts->preetyUrl($_GET['title']) ?>"</script>
      <?php 
     ...
     exit;
         ...
         $_SESSION['name'] = $dbusername;
      echo "<script>window.location.href = 'subscriber/index.php';</script>";
       exit;
